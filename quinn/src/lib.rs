...
impl Future for Driver {
    ...
    fn poll(&mut self) -> Poll<Self::Item, Self::Error> {
        ...
        loop {
            ...
            keep_going |= endpoint.poll_recv(now, self.0.clone())?;
            endpoint.poll_events();
            endpoint.poll_incoming(self.0.clone());
            let _ = endpoint.incoming.poll_complete();

            if !endpoint.poll_send()? {
                keep_going |= endpoint.poll_transmit(now)?;
            }

            keep_going |= endpoint.poll_timers(now)?;
...
impl EndpointInner {
    ...
    fn poll_incoming(&mut self, wrapped: Rc<RefCell<EndpointInner>>) {
    ...
    fn poll_send(&mut self) -> Result<bool, io::Error> {
    ...
    fn poll_transmit(&mut self, now: Instant) -> Result<bool, io::Error> {
        let mut sent = 0;
        let mut keep_going = false;
    ...
    fn poll_timers(&mut self, now: Instant) -> Result<bool, io::Error> {
        let mut keep_going = false;
        loop {
